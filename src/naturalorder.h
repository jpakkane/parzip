...
template <class T> try_result tryint(T &b, T &e) {
    try_result r;
    r.was_num = false;
    r.value = 0;
    r.has_next = true;
...
template <class T> int natural_compare(T str1_begin, T str1_end, T str2_begin, T str2_end) {
    while (true) {
 ...
 */
template <class T1, class T2> int natural_compare(T1 &str1, T2 &str2) {
 ...
 */
template <class T> int natural_less(T &s1, T &s2) { return natural_compare(s1, s2) < 0; }
